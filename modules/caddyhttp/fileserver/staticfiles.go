...
func (fsrv *FileServer) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	// logger.Infof("received request url = <%v>", url)

	// 修改 RequestURL 以便和给定序列中的 URL 相同
	// var url string
	// if filename == "." {
	// 	url = "index.html"
	// } else {
	...
	// TODO: 保存文件名，稍后打开。直接传递的 file 指针会在函数结束之后被 defer 关闭掉
	caddy.AddNewResponseWriterControlBlock(w, r, info.Name(), info.ModTime(), file)

   	for true {
		select {
		case signal := <- caddy.MoveOnChan:
			if signal == url {
				// 本 ResponseWriter 已经结束，退出此函数
				goto moveOn
			}
		}
	}
